...
from app.schemas.enums import InventoryMoveType, PurchaseRequestStatus, UserRole, WorkOrderStatus
...
def _ot_row(row: dict) -> WorkOrderRead:
    return WorkOrderRead.model_validate(row)
...
async def assign_technician(
    ...
    _require_roles(current_user, UserRole.asesor_servicio, UserRole.administrador)
    await _fetch_work_order(client, ot_id)
...
async def register_diagnostic(
    ...
    if current_user.role == UserRole.tecnico and ot.tecnico_id and ot.tecnico_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="La OT esta asignada a otro tecnico.")
...
async def change_work_order_status(
    ...
    if target in {WorkOrderStatus.waiting_parts, WorkOrderStatus.completed, WorkOrderStatus.cancelada}:
        _require_roles(current_user, UserRole.asesor_servicio, UserRole.administrador)
    elif target in {WorkOrderStatus.in_progress, WorkOrderStatus.tech_completed}:
        _require_roles(current_user, UserRole.tecnico, UserRole.asesor_servicio, UserRole.administrador)
...
async def list_prs(
    ...
    *,
    ...
    items = [await _fetch_purchase_request(client, row["id"]) for row in response.data or []]
